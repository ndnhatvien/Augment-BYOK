(function () {
  ...
  function computeThinkingUi({ type, requestDefaults }) {
    ...
    if (type === "anthropic") {
      ...
      return { supported: true, value: v, hint: "Anthropic: write requestDefaults.thinking.budget_tokens (Low/Medium/High/xhigh)" };
    }

    return { supported: false, value: "", hint: "Not supported for this type (you can customize in Defaults JSON)" };
  }
        ...
        return `
            <div class="provider-card${isExpanded ? " is-expanded" : ""}" data-provider-card data-provider-idx="${idx}" data-provider-key="${escapeHtml(pKey)}">
              <div class="provider-card__header" data-action="toggleProviderCard" data-idx="${idx}">
                ...
                <div class="flex-row flex-wrap">
                  <button class="btn btn--small" data-action="makeProviderDefault" data-idx="${idx}" ${idx === 0 ? "disabled" : ""}>Make Default</button>
                  <button class="btn btn--small btn--danger" data-action="removeProvider" data-idx="${idx}">Delete</button>
                </div>
              </div>
                    ...
                    <div class="form-grid">
                      ...
                      <div class="form-group">
                        ...
                        <div class="flex-row flex-wrap">
                          ...
                          <button class="btn btn--small" data-action="fetchProviderModels" data-idx="${idx}">Fetch Model</button>
    ...
    return `
        <section class="settings-panel">
          ...
          <div class="settings-panel__body">
            <div class="text-muted text-xs">Convention: first item (<span class="text-mono">providers[0]</span>) is the default BYOK provider.</div>
            <div style="height:8px;"></div>
            <div class="provider-list">
              ${list || `<div class="text-muted" style="text-align:center;padding:20px;">No Provider yet, click Add in the top-right.</div>`}
            </div>
          </div>
        </section>
      `;
