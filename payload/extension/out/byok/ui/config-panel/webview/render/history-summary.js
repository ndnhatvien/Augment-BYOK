	        ...
	        <div class="settings-panel__body">
	          <div class="form-grid">
	            <div class="form-group">
	              ...
	              <label class="checkbox-wrapper"><input type="checkbox" id="historySummaryEnabled" ${hsEnabled ? "checked" : ""} /><span>Enable</span></label>
	              <div class="text-muted text-xs">When enabled, rolling summary runs in the background to avoid context overflow (only affects content sent to upstream model).</div>
	            </div>
	            <div class="form-group">
	              ...
	              <div class="text-muted text-xs">Empty = follow current request model (for summary generation only); trigger window check always uses current chat model.</div>
	            </div>
	            ...
	            <div class="form-group form-grid--full">
	              <details class="endpoint-group">
	                ...
	                <div class="endpoint-group-body">
	                  <div class="text-muted text-xs">Advanced parameters are optional; empty falls back to defaults. Built-in covers common coding models (Claude4/GPT5/Gemini/Kimi).</div>
	                  ...
	                  <div class="form-grid">
	                    ...
	                    <div class="form-group"><label class="form-label">Rolling Summary</label><label class="checkbox-wrapper"><input type="checkbox" id="historySummaryRollingSummary" ${hsRollingSummary ? "checked" : ""} /><span>Enable incremental cache summary</span></label></div>
	                    <div class="form-group form-grid--full"><label class="form-label" for="historySummaryContextWindowTokensOverrides">Context Window Overrides (JSON object)</label><textarea class="mono" id="historySummaryContextWindowTokensOverrides" rows="6" placeholder='{"gpt-5.3-codex":400000,"gpt-5.2":400000,"claude-4.6-opus":1000000,"gemini-3-pro":1000000,"kimi-k2":128000}'>${escapeHtml(hsContextWindowTokensOverrides)}</textarea><div class="text-muted text-xs">Matched by substring of current chat model name, longer match wins; case-insensitive. Does not change the summary model.</div></div>
	                    <div class="form-group form-grid--full"><label class="form-label" for="historySummaryPrompt">Prompt</label><textarea class="mono" id="historySummaryPrompt" rows="6" placeholder="(default)">${escapeHtml(hsPrompt)}</textarea><div class="text-muted text-xs">Keep concise and structured; avoid leaking sensitive info. Empty falls back to default template.</div></div>
	                  </div>
	                </div>
	              </details>
	            </div>
