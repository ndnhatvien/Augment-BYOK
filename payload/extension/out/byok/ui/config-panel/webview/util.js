(function () {
  ...
  ns.validateHttpUrl = function validateHttpUrl(raw) {
    ...
    if (protocol !== "http:" && protocol !== "https:") return { ok: false, empty: false, href: "", error: `Only http/https supported (current: ${u.protocol || "unknown"})` };
  ...
  ns.validateProviderDraft = function validateProviderDraft(provider) {
    ...
    if (!id) push("warning", "id", "ID is empty: after saving this Provider will be dropped (normalizeConfig requires non-empty id/type).");
    ...
    if (baseCheck.ok && baseCheck.empty) {
      push("warning", "baseUrl", "Base URL is empty: this Provider cannot make requests (suggest an http(s) address).");
      ...
      if (pathname && !pathname.includes("/v1")) push("warning", "baseUrl", "Base URL usually contains /v1 (can be ignored if your service does not need it).");
    }

    if (!models.length && !defaultModel) {
      push("warning", "models", "Models is empty: click \"Fetch\" or \"Edit\" to set at least one model / default model.");
    } else if (defaultModel && models.length && !models.includes(defaultModel)) {
      push("warning", "models", "Default Model is not in the Models list (may be rewritten on save/normalize).");
    }
