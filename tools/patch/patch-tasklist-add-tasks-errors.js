...
function patchTasklistAddTasksErrors(filePath) {
  ...
  next = replaceOnceRegex(
    ...
    (m) => {
      ...
      const formatClass = String(m[5] || "");
      const diffFunc = String(m[6] || "");
      const beforeVar = String(m[7] || "");
      const afterVar = String(m[8] || "");
      const wrapFunc = String(m[9] || "");
      if (!resultsVar || !textVar || !beforeVar || !afterVar) throw new Error("tasklist add_tasks errors: capture missing");

      const oldTail = `let ${textVar}=${formatClass}.formatBulkUpdateResponse(${diffFunc}(${beforeVar},${afterVar}));return{...${wrapFunc}(${textVar}),plan:${afterVar}}`;
      const insertion =
...
` +
        ...
        `if(__byok_failed.length===${resultsVar}.length)return{...et("Failed to add task(s)."+__byok_msg),plan:${afterVar}};` +
        `${textVar}+=__byok_msg;` +
        `}`;

      const newTail = `let ${textVar}=${formatClass}.formatBulkUpdateResponse(${diffFunc}(${beforeVar},${afterVar}));${insertion}return{...${wrapFunc}(${textVar}),plan:${afterVar}}`;
