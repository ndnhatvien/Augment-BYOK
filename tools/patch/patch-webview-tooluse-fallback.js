...
function patchAugmentMessageAsset(filePath) {
  ...
  if (!out.includes(MARKER_TOOL_LIST_UNGROUPED)) {
    ...
    if (!alreadyPatched) {
      out = replaceOnceRegex(
        out,
        /([A-Za-z_$][0-9A-Za-z_$]*)=([A-Za-z_$][0-9A-Za-z_$]*)\(\(\(\)=>([a-z]+)\(e\(([A-Za-z_$][0-9A-Za-z_$]*)\),"\$displayableToolUseNodes",([A-Za-z_$][0-9A-Za-z_$]*)\)\.map\(\(([A-Za-z_$][0-9A-Za-z_$]*)=>\6\.tool_use\)\)\.filter\(\(\6=>!!\6\)\)\)\);/g,
        (m) =>
  ...
  if (!out.includes(MARKER_TOOL_STATE)) {
    ...
    if (!alreadyPatched) {
      out = replaceOnceRegex(
        out,
        /([A-Za-z_$][0-9A-Za-z_$]*)=\(\)=>([a-z]+)\(e\(([A-Za-z_$][0-9A-Za-z_$]*)\),"\$toolUseState",([A-Za-z_$][0-9A-Za-z_$]*)\)/g,
        (m) =>
